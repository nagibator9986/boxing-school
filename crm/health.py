...
#: Через сколько часов молчания диалог считается забытым. Сутки оказались
#: величиной не из той жизни: 03.09.2026 клиент спросил «оплата какая?» и ждал
#: ответа тридцать семь минут — в переписке о покупке это уже потерянный клиент.
STALE_PAUSE_HOURS: Final[int] = 2
...
def _forgotten_dialogs(bot) -> list[Issue]:
    ...
    return [
        Issue(
            "warn",
            f"Клиенты ждут ответа: {stale}",
            f"В этих диалогах бот замолчал, потому что писал человек, и с последнего "
            f"сообщения клиента прошло больше {STALE_PAUSE_HOURS} ч. Ответьте сами или "
            f"верните бота кнопкой в карточке клиента.",
        )
    ]
