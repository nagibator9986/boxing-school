...
#: Идентификатор группового чата WhatsApp — целиком, с доменом групп ``@g.us``:
#: «77010000000-1600000000@g.us» (номер создателя и отметка времени) или длинный
#: номер сообщества «120363012345678901@g.us». Ровно в таком виде его отдаёт Wazzup.
#:
#: Проверка нарочно узкая — всё, что шире, ломает живые записи владельца:
#: «Zarina +7 777 000 00 01» — номер с подписью, а не чат; «77012345678@c.us» —
#: личный адрес абонента; «87010000001-87010000002» — два номера через дефис.
#: Обрезанный до «77010000000-1600000000» идентификатор группой не считается:
#: ничего не уйдёт и это будет видно, а вот отправка не в тот чат — нет.
_GROUP_CHAT_RE: Final[re.Pattern[str]] = re.compile(r"^[\w.\-]+@g\.us$", re.IGNORECASE)


def is_group_chat(value: str | None) -> bool:
    """Групповой чат WhatsApp, а не номер абонента."""
    return bool(_GROUP_CHAT_RE.match((value or "").strip()))
