...
def build_dynamic_note(
    *,
    ...
    if cards_sent:
        # Карточки собирает код и отправляет их сам. Пересказ того же своими
        # словами клиент читает как второй экземпляр — и это ещё и риск ошибки:
        # в пересказе теряются строки и путаются дни.
        lines.append(
            "Клиент в этом диалоге уже получил готовые карточки: "
            + ", ".join(cards_sent)
            + ". Не пересказывай их содержимое и не присылай повторно — "
            "ссылайся коротко («как в списке выше») и веди разговор дальше."
        )
