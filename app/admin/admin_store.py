...
class SettingSpec:
    ...
    key: str
    title: str
    kind: str  # bool | time_range | minutes | phones | text
    default: str
    hint: str
...
SETTING_SPECS: Final[tuple[SettingSpec, ...]] = (
    ...
    SettingSpec(
        ...
        default="0",
        hint=(
            "Сколько минут бот молчит в диалоге, где ответил человек. "
            "0 — молчать, пока бота не вернут вручную: строкой #бот в переписке "
            "или кнопкой «Вернуть бота» в карточке клиента. Так разговор, "
            "начатый человеком, остаётся за человеком."
        ),
    ),
    SettingSpec(
        key="ignored_numbers",
    ...
    SettingSpec(
        key="auto_greeting_texts",
        title="Тексты автоответов WhatsApp",
        kind="text",
        default="ЖМИ ОТПРАВИТЬ\nThank you for contacting us",
        hint=(
            "Что WhatsApp Business отправляет сам: приветствие в чате из рекламы, "
            "автоответ «мы скоро свяжемся», сообщение вне рабочих часов. Такие "
            "сообщения бот не считает вашим вмешательством и продолжает работать. "
            "Любое другое исходящее означает, что в диалог вошли вы, — и бот "
            "сразу замолкает. Достаточно узнаваемого куска; варианты — с новой строки."
        ),
    ),
    SettingSpec(
        key="lead_notify",
    ...
    SettingSpec(
        key="lead_notify_target",
        title="Номер для заявок",
        kind="phones",
        default="",
        hint=(
            "WhatsApp администратора, куда бот присылает карточки готовых записей и "
            "вопросов, на которые нужен человек. Можно несколько номеров через запятую — "
            "карточка уйдёт каждому. Пусто — карточки идут на номер из настроек сервера "
            "(MANAGER_NOTIFY_TARGET). На сообщения с этих номеров бот не отвечает."
        ),
    ),
    SettingSpec(
        key="lead_notify_chat",
        title="Чат для коротких уведомлений",
        kind="phones",
        default="",
        hint=(
            "Куда бот одной строкой пишет о каждой готовой записи: «Записался: имя — зал, "
            "когда». Номер или идентификатор группового чата из Wazzup — целиком, вместе "
            "с «@g.us» (77010000000-1600000000@g.us). Телефон клиента в эту строку не "
            "попадает, и на сообщения из этого чата бот не отвечает."
        ),
    ),
    SettingSpec(
        key="trial_free",
