...
SETTING_SPECS: Final[tuple[SettingSpec, ...]] = (
    ...
    SettingSpec(
        ...
        title="Текст автоприветствия",
        kind="text",
        default="ЖМИ ОТПРАВИТЬ",
        hint=(
            "Что WhatsApp Business отправляет сам в чат, открытый из рекламы. "
            "Такое сообщение бот не считает вмешательством человека. Любое "
            "другое исходящее из аккаунта означает, что в диалог вошли вы, — и "
            "бот сразу замолкает. Несколько вариантов — с новой строки."
        ),
    ),
    SettingSpec(
        key="lead_notify",
