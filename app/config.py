...
class Settings(BaseSettings):
    ...
    #: Сколько ждать, вдруг клиент дописывает мысль вторым сообщением. Замер
    #: 03.09.2026: пять секунд — самая крупная задержка ответа, больше обоих
    #: вызовов модели вместе. Две с половиной склеивают «Здравствуйте» и
    #: «хочу записать» и не заставляют ждать того, кто написал всё сразу.
    debounce_seconds: float = 2.5
    debounce_max_seconds: float = 6.0
    ...
    #: Отправлять ли первую карточку хода, не дожидаясь текста модели. Это
    #: минус секунда на вопросах о ценах и расписании, но карточка и вопрос
    #: приходят двумя сообщениями вместо одного. Файл (видео дороги) уходит рано
    #: всегда: он и так отдельное сообщение.
    send_cards_early: bool = False
    #: Пауза перед вторым сообщением хода: чтобы карточка и текст не слипались
    #: в одно уведомление. Секунды хватает, две ощущались как задержка.
    second_message_delay_ms: int = 1000
    #: Сообщения одного чата уходят строго по очереди: если более раннее ещё не
    #: ушло, следующее переставляется на столько миллисекунд.
    outbox_order_retry_ms: int = 700
    #: После файла (видео дороги) следующее сообщение чата ждёт столько секунд:
    #: Wazzup ещё скачивает видео, и короткий текст, отправленный сразу, его
    #: обгоняет. Увеличить, если клиенты видят текст раньше видео.
    media_settle_seconds: float = 8.0
    #: Сколько бот молчит после ответа человека. Задаётся владельцем в CRM
    #: («Пауза бота после ответа менеджера»); здесь — запасное значение.
    #: Ноль означает «молчать, пока бота не вернут вручную» — кнопкой в CRM или
    #: командой в чат. Сначала было тридцать минут, потом сто двадцать, и каждый
    #: раз оказывалось мало: человек ведёт разговор с перерывами, а бот
    #: возвращался посреди него. Владелец: «бот должен резко затихать».
    pause_operator_minutes: int = 0
    ...
    #: Через сколько суток молчания клиент считается вернувшимся, и меню из
    #: четырёх пунктов показывается снова. Ноль — никогда не повторять.
    #: Меню отвечает на вопрос «что писать дальше», и у человека, который писал
    #: месяц назад, этот вопрос ровно тот же, что у нового. Без правила «один
    #: раз на диалог» означало «один раз навсегда»: вернувшийся получал общий
    #: ответ модели вместо списка вариантов.
    greeting_repeat_days: int = 7
    #: Номера, которым бот не отвечает: тренеры и сотрудники пишут в тот же
    #: WhatsApp, что и родители. Разделители — запятая, пробел, перевод строки;
    #: сравнение идёт по последним десяти цифрам, поэтому формат записи не важен.
    #: Значение задаётся владельцем в CRM и переменной окружения не требует.
    ignored_numbers: str = ""
    #: Текст автоприветствия, которое WhatsApp Business шлёт сам в чат из
    #: рекламы. Такое исходящее — не человек, вошедший в диалог, и глушить бота
