...
#: а не в коде рендера.
#: ``funnel`` — наводящие вопросы, которыми бот ведёт разговор к записи. Их
#: подставляет код, когда модель закончила ответ без вопроса, поэтому они тоже
#: обязаны быть на обоих языках и правиться владельцем без правки кода.
I18N_GROUPS: Final[frozenset[str]] = frozenset(
    {
        "gap", "escalation", "error", "bridge", "followup", "lead_card",
        "greeting", "system", "card", "funnel",
    }
)
