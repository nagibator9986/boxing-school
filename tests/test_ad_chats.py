...
# --------------------------------------------------------------------------- #
# Приветствие с упоминанием рекламы
# --------------------------------------------------------------------------- #
@pytest.mark.parametrize(
    ...
    [
        ...
        "Привет, я по рекламе",
        "Здравствуйте, нашла вас в инстаграме",
        "Здравствуйте, увидел вашу рекламу",
    ],
)
def test_ad_opener_still_gets_the_menu(text: str) -> None:
    """«Здравствуйте, увидела рекламу» — это приветствие, а не вопрос.

    Клиент с таргета ничего не спросил, и меню из четырёх пунктов нужно ему так
    же, как поздоровавшемуся молча. Без этого модель сочиняла собственное меню —
    с другими пунктами и на двух языках сразу.
    """
    from app.core.pipeline import _is_bare_greeting

    assert _is_bare_greeting(text)


@pytest.mark.parametrize(
    "text",
    [
        "Здравствуйте, увидела рекламу, сколько стоит абонемент?",
        "Здравствуйте! По объявлению. Ребёнку 7 лет, куда подойти?",
        "Здравствуйте, хочу записать ребёнка",
    ],
)
def test_question_after_the_ad_mention_is_answered_not_menued(text: str) -> None:
    """Спросил — получает ответ на вопрос, а не список пунктов."""
    from app.core.pipeline import _is_bare_greeting
