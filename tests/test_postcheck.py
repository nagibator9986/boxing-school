...
# --------------------------------------------------------------------------- #
# Возрастная граница приёма
# --------------------------------------------------------------------------- #
@pytest.mark.parametrize(
    "text",
    [
        "По возрасту группы на кикбоксинг рассчитаны на детей с 7 лет, а Бекзату сейчас 6.",
        "Да, в младшие группы мы берём детей с 7 лет.",
        "Записываем с 8 лет.",
    ],
)
def test_invented_age_limit_is_blocked(text: str, kb) -> None:
    """Живой случай 03.09.2026: бот отказал шестилетнему.

    Владелец: «такую информацию мы не давали, у нас дети с 5 лет». Числа в такой
    фразе есть, но обычные проверки их пропускают: возраст — не деньги и не
    время, и в данных инструментов его не было.
    """
    result = verdict(text, kb)
...
@pytest.mark.parametrize(
    "text",
    [
        "Принимаем детей с 5 лет. Скажите, сколько лет ребёнку.",
        "Берём с 5 лет и ведём группами по возрасту.",
        "Ему 6 лет — передам вопрос администратору, он подскажет по группе.",
        "Записал сына, ему 9 лет.",
    ],
)
def test_correct_age_and_the_child_age_pass(text: str, kb) -> None:
    """Подтверждённая граница и возраст самого ребёнка проходят.

    Правило обязано ловить выдуманную границу приёма, а не любое число рядом со
    словом «лет»: снятый верный ответ — это молчащий бот.
    """
...
@pytest.mark.parametrize(
    "text",
    [
        "Спасибо, что предупредили — я передам тренеру.",
        "Хорошо, сообщу тренеру о пропуске.",
        "Предупрежу тренера, что вы задержитесь.",
    ],
)
def test_promise_to_tell_the_coach_is_blocked(text: str, kb) -> None:
    """Обещание, которое некому исполнить.

    У бота нет ни номера тренера, ни канала до него: всё, что он умеет, — позвать
    администратора. 03.09.2026 родителю, предупредившему о пропуске, он ответил
    «передам тренеру». Никто ничего не передал, и ребёнка ждали на тренировке.
    """
    assert not verdict(text, kb).ok


def test_handing_over_to_the_administrator_is_fine(kb) -> None:
    """Администратора бот звать умеет — это обещание он держит."""
    assert verdict("Передаю администратору — он ответит здесь же.", kb).ok
...
@pytest.mark.parametrize(
    "text",
    [
        "Рады записать Айсулу на пробное занятие.",
        "Ждём Бекзата на тренировке.",
        "Приведёте Данияра к 19:00 — мы встретим.",
        "Здравствуйте, Айгуль! С удовольствием поможем.",
        "Добрый день, Данияр! Записываем на пробное.",
    ],
)
def test_invented_name_is_blocked(text: str, kb) -> None:
    """Имени клиент не называл — значит, бот его придумал.

    03.09.2026 на «хочу записать дочь на пробное» бот ответил «рады записать
    Айсулу». Такого имени не было ни в переписке, ни в базе знаний: для родителя
    это выглядит как переписка о чужом ребёнке.
    """
    assert not verdict(text, kb).ok
...
@pytest.mark.parametrize(
    ("text", "known"),
    [
        ("Рады записать Айсулу на пробное занятие.", ("Айсулу",)),
        ("Ждём Бекзата на тренировке.", ("Бекзат",)),
        ("Рады записать вашу дочь на пробное занятие.", ()),
        ("Записываем ребёнка — в каком районе удобнее?", ()),
        ("Хорошо, записываем вас на пробное.", ()),
        ("Конечно, передаю администратору.", ()),
        ("Здравствуйте, Айгуль! С удовольствием поможем.", ("Айгуль",)),
        ("Здравствуйте! Это школа бокса AINAZAROV TOP TEAM.", ()),
        ("Здравствуйте, подскажите возраст ребёнка.", ()),
    ],
)
def test_known_name_and_nameless_reply_pass(text: str, known: tuple[str, ...], kb) -> None:
    """Названное имя — не выдумка, а вежливость; безымянный ответ трогать нечем.

    «Бекзата» и «Бекзат» — одно имя: сверка идёт по основе, иначе бот не смог бы
...
# --------------------------------------------------------------------------- #
# Место школы против выдуманного города
# --------------------------------------------------------------------------- #
@pytest.mark.parametrize(
    "text",
    ["Наш зал в Тобыле работает по будням.", "Зал на Арыстанбекова ждёт вас."],
)
def test_place_from_the_knowledge_base_passes(text: str, kb) -> None:
    """Посёлок и улица из базы — правда, даже если инструмент их не возвращал.

    03.09.2026 на «мы в Тобыле, сколько стоит?» бот посчитал цену, но ответ был
    снят: цену инструмент вернул, а название посёлка — нет, и «зал в Тобыле»
    посчиталось выдуманным филиалом. Проверка задумывалась как «нет в реестре
    KB», а сверялась только с данными хода.
    """
    assert verdict(text, kb).ok
...
@pytest.mark.parametrize(
    "text",
    [
        "Наш зал в Астане ждёт вас.",
        "Филиал в Алматы работает круглый год.",
        "Есть зал в Затобольске, приходите туда.",
    ],
)
def test_invented_place_is_still_blocked(text: str, kb) -> None:
    """Чужой город — выдумка, и район без зала тоже: «Затобольск» знаком, зала нет.
